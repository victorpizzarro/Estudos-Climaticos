...
def hamed_rao_mk(
    ...
    # IMPORTANTE:
    # Na implementação Hamed-Rao usada como referência
    # (pyMannKendall), a soma ponderada das autocorrelações
    # entra em módulo. Isso impede que autocorrelações
    # negativas reduzam artificialmente a variância do teste
    # e produzam significâncias espúrias.
    #
    # n_ns = 1 + [2 / (n(n-1)(n-2))] * abs(sni)
    #
    # Logo, o fator de correção deve ser >= 1.
    fator_correcao = (
        1.0
        +
        (
    ...
    # Validação defensiva: com a formulação acima,
    # o fator nunca deve ser menor que 1, salvo ruído
    # numérico ínfimo.
    if (
        not np.isfinite(
            fator_correcao
        ...
        or
        fator_correcao < 1.0 - 1e-12
    ):

        raise RuntimeError(
            "Fator Hamed-Rao inválido: "
            f"{fator_correcao}"
        )
...
print("=" * 90)
print("MANN-KENDALL MODIFICADO PARA AUTOCORRELAÇÃO - REVISADO")
print("=" * 90)
